...
using System.Linq;
using FreeSql;
using Huan.DbSwitcher.FreeSqlStore.Uow;
...
namespace Huan.DbSwitcher.FreeSqlStore.Repositories
{
        ...
        where TEntity : class, IDbEntity<TPrimaryKey>
    {
        ...
        protected readonly FreeSqlRepositoryResolver FreeSqlRepositoryResolver;
        ...
        FreeSqlRepositoryResolver RepositoryResolver
            => this.FreeSqlRepositoryResolver;
        IPartitionTableNameFactory PartitionTableNameFactory
            => this.FreeSqlRepositoryResolver.PartitionTableNameFactory;
        IRelationalDatabaseProcessor RelationalDatabaseProcessor
            => this.FreeSqlRepositoryResolver.RelationalDatabaseProcessorDbStore.GetByName(ProviderName, FreeSqlDbStoreConsts.DefaultProviderName);


        public FSqlRepository(FreeSqlRepositoryResolver freeSqlRepositoryResolver)
        {
            FreeSqlRepositoryResolver = freeSqlRepositoryResolver;
        ...
        {
            return RepositoryResolver.GetFreeSql(this.ProviderName);
        }
        ...
        public ISelect<TEntity> GetSelect(DateTime start, DateTime end)
        {
            ...
            // 开始分表名称和结束分表名称
            var startTableName = this.PartitionTableNameFactory.GetTableName(this.EntityName, start);
            var endTableName = this.PartitionTableNameFactory.GetTableName(this.EntityName, end);
            ...
            while (startTableName != endTableName)
            {
                ...
                startTableName = this.PartitionTableNameFactory.GetTableName(this.EntityName, nextDateKey);
                resultSelect = GetSelectPrivate(resultSelect, startTableName);
            }
        ...
        public IQueryable<TEntity> GetQuery(object partitionId = null)
        {
            ...
            return  RepositoryResolver
                .GetRepository<TEntity, TPrimaryKey>(partitionId, this.ProviderName, unitOfWork).Select.AsQueryable();
        ...
        public TEntity Insert(TEntity entity, object partitionId = null)
        {
            ...
            return this.GetTable(null).Insert(entity);
        }
        ...
        public TEntity Update(TEntity entity, object partitionId = null)
        {
            ...
            {
                // ReSharper disable once RedundantArgumentDefaultValue
                this.GetTable(null).Update(entity);
            }
        ...
        private string GetUnitOfWorkKey(string name = null)
        {
            ...
            return string.IsNullOrWhiteSpace(this.ProviderName) ? FreeSqlDbStoreConsts.DefaultProviderName : this.ProviderName;
        }
